...
router.post("/createuser", [
    ...
    body("cpassword", "Please! Enter your Password Again!").isLength({ min: 8, max: 16 })
], async (req, res) => {
    ...
    try {
        ...
        const securePassword2 = await bcrypt.hash(req.body.cpassword, salt)

        // chech whether user with this email exists already
        let user = await User.findOne({ email: req.body.email })
        if (user) {
            return res.status(400).json({ success, error: "E-mail already exist! Please enter a new one." });
        }
        ...
        if (securePassword === securePassword2) {
            ...
            return res.status(401).json({ success, error: "Password and Confirm Password mismatch! Enter Correctly." });
        }
    }
    catch (error) {
